...
//        view.backgroundColor = UIColor(named: "AppBackground")
    ...
    private func setupCollectionView() {
        ...
        // Build the basic flow configuration structure once
...
extension SportsViewController: UICollectionViewDelegateFlowLayout {
    ...
    // HIGH-PERFORMANCE FIX: Let UIKit poll the size dynamically per item index safely
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        
        let itemWidth = collectionView.bounds.width - (sideInset * 2)
        
        // Mathematical split to force exactly 3 items per visible viewport height safely
